...
namespace ReviveUSBAdvance
{
    class EepromControl
    {
        ...
        /// 全体設定情報
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        public const int E2P_ALL_SETTING_INFO_AREA_ALL_SIZE = 0x000005;   // 全体設定情報 全サイズ
        public const int E2P_ADR_ALL_SETTING_INFO           = 0x000002;   // 全体設定情報 保存先頭アドレス
        public const int E2P_ALL_SETTING_INFO_AREA_SIZE     = 0x000005;   // 全体設定情報 保存エリアサイズ
        public const int E2P_ALL_SETTING_INFO_ALL_DATA_SIZE = 0x000001;   // 全体設定情報 DATAサイズ
        ...
        public const uint E2P_READ_TYPE_ALL_SETTING         = 0x0004;     // 
        public const uint E2P_READ_TYPE_SW_SETTING          = 0x0001;     // 
        public const uint E2P_READ_TYPE_AN_SETTING          = 0x0002;     // 
        public const uint E2P_WRITE_TYPE_ALL_SETTING        = 0x0004;     // 
        ...
        public EepromControl()
        {
            ...
            read_flag_all_setting = E2P_READ_STATUS_NA;
            ...
            write_flag_all_setting = E2P_WRITE_STATUS_NA;
        ...
        public void Set_Eeprom_Read_Status(uint read_type, byte set_status)
        {
            try
            {
                switch (read_type)
                {
                    case E2P_READ_TYPE_ALL_SETTING:
                        if (set_status == E2P_READ_STATUS_RQ
                            && (read_flag_all_setting == E2P_READ_STATUS_NA || read_flag_all_setting == E2P_READ_STATUS_COMP))
                        {
                            read_flag_all_setting = E2P_READ_STATUS_RQ;
                        }
                        if (set_status == E2P_READ_STATUS_READING
                            && (read_flag_all_setting == E2P_READ_STATUS_RQ))
                        {
                            read_flag_all_setting = E2P_READ_STATUS_READING;
                        }
                        if (set_status == E2P_READ_STATUS_COMP
                            && (read_flag_all_setting == E2P_READ_STATUS_READING))
                        {
                            read_flag_all_setting = E2P_READ_STATUS_COMP;
                        }
        ...
        public void Set_Eeprom_Read_Status_Clear(uint read_type)
        {
            try
            {
                if ((read_type & E2P_READ_TYPE_ALL_SETTING) != 0)
                {
                    read_flag_all_setting = E2P_READ_STATUS_NA;
                }
        ...
        public bool Set_Eeprom_Read_ALL_Info(ref Form1.STR_ALL_SETTING_DATAS p_all_datas, ArrayList p_al)
        {
            bool b_ret = false;
            try
            {
                if (p_al.Count == E2P_ALL_SETTING_INFO_AREA_ALL_SIZE)
                {
                    // 全体設定データをセット
                    p_all_datas.sw_smpl_interval     = (byte)p_al[E2P_ALL_SETTING_INFO_ALL_DATA_SIZE * 0];
                    p_all_datas.sw_check_count       = (byte)p_al[E2P_ALL_SETTING_INFO_ALL_DATA_SIZE * 1];
                    p_all_datas.sw_renc_conv_enables = (byte)p_al[E2P_ALL_SETTING_INFO_ALL_DATA_SIZE * 2];
                    p_all_datas.sw_renc_check_count  = (byte)p_al[E2P_ALL_SETTING_INFO_ALL_DATA_SIZE * 3];
                    p_all_datas.an_smpl_interval     = (byte)p_al[E2P_ALL_SETTING_INFO_ALL_DATA_SIZE * 4];
                }
            }
            catch
            {
        ...
        public void Set_Eeprom_Write_Status(uint write_type, byte set_status)
        {
            try
            {
                switch (write_type)
                {
                    case E2P_WRITE_TYPE_ALL_SETTING:
                        if (set_status == E2P_WRITE_STATUS_RQ
                            && (write_flag_all_setting == E2P_WRITE_STATUS_NA || write_flag_all_setting == E2P_WRITE_STATUS_COMP))
                        {
                            write_flag_all_setting = E2P_WRITE_STATUS_RQ;
                        }
                        if (set_status == E2P_WRITE_STATUS_WRITTING
                            && (write_flag_all_setting == E2P_WRITE_STATUS_RQ))
                        {
                            write_flag_all_setting = E2P_WRITE_STATUS_WRITTING;
                        }
                        if (set_status == E2P_WRITE_STATUS_COMP
                            && (write_flag_all_setting == E2P_WRITE_STATUS_WRITTING))
                        {
                            write_flag_all_setting = E2P_WRITE_STATUS_COMP;
                        }
        ...
        public void Set_Eeprom_Write_Status_Clear(uint write_type)
        {
            try
            {
                if ((write_type & E2P_WRITE_TYPE_ALL_SETTING) != 0)
                {
                    write_flag_all_setting = E2P_WRITE_STATUS_NA;
                }
