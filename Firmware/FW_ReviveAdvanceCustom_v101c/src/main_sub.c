...
// グレイコード変換テーブル
const char renc_gray_code_conv_table[4][4] = {
    { 0, 1,-1,-2}
   ,{-1, 0,-2, 1}
   ,{ 1,-2, 0,-1}
   ,{-2,-1, 1, 0}
};
WORD renc_pre_r_code = 0;
WORD renc_pre_result = 0;
...
void Switch_Input(void)
{
    ...
    {
        // ロータリーエンコーダ設定をしているSWの場合（D1/D2に設定するとSW1 or SW2が当てはまる）
        if(((my_all_infos.ALL_Infos.sw_renc_conv_enables & (0x01 << (fi/2))) ? 1:0))
        {
            tmp_check_count = my_all_infos.ALL_Infos.sw_renc_check_count;
            
            // A相/B相をまとめて処理
            if (fi % 2 == 0)
            {
              // sw_now[fi]=A相、sw_now[fi+1]=B相なので、A相/B相の並び順になるように連結する
              tmp_gray_code = !sw_now[fi+1] | (!sw_now[fi] << 1);
              // 回転方向の検知（前回のコードと今回のコードを比較）
              switch(renc_gray_code_conv_table[(renc_pre_r_code >> fi) & 0x0003][tmp_gray_code])
              {
                  case 0: // 静止（A相＝0、B相＝0として扱う）
                      sw_now[fi]   = R_OFF;
                      sw_now[fi+1] = R_OFF;
                      renc_pre_result &= ~((WORD)0x0003 << fi);
                      break;
                  case 1: // 正転（A相＝1、B相＝0として扱う）
                      sw_now[fi]   = R_ON;
                      sw_now[fi+1] = R_OFF;
                      renc_pre_result &= ~((WORD)0x0003 << fi);
                      renc_pre_result |= ((WORD)0x0001 << fi);
                      break;
                  case -1: // 逆転（A相＝0、B相＝1として扱う）
                      sw_now[fi]   = R_OFF;
                      sw_now[fi+1] = R_ON;
                      renc_pre_result &= ~((WORD)0x0003 << fi);
                      renc_pre_result |= ((WORD)0x0002 << fi);
                      break;
                  case -2: // 無効（オーバースピードエラー。読みこぼしを防ぐために前回と同じように扱う）
                      sw_now[fi]   = (!renc_pre_result >> fi) & 0x0001;
                      sw_now[fi+1] = (!renc_pre_result >> fi) & 0x0002;
                      break;
              }
              // 次の回転方向の検知に使用するコードを保存
              renc_pre_r_code &= ~((WORD)0x0003 << fi);
              renc_pre_r_code |= (tmp_gray_code << fi);
            }
        }
        else
        {
            tmp_check_count = my_all_infos.ALL_Infos.sw_check_count;
        }
        ...
        {
            sw_press_on_cnt[fi]++;
            if(sw_press_on_cnt[fi] >= tmp_check_count)
            {
                sw_now_fix[fi] = N_ON;
                sw_press_on_cnt[fi] = tmp_check_count;
        ...
        {
            sw_press_off_cnt[fi]++;
            if(sw_press_off_cnt[fi] >= tmp_check_count)
            {
                sw_now_fix[fi] = N_OFF;
                sw_press_off_cnt[fi] = tmp_check_count;
            }
...
void Check_AllSettingInfo(void)
{
    // SW設定 デジタルサンプリング周期
    if (my_all_infos.ALL_Infos.sw_smpl_interval <= 0x0000)
    {
        my_all_infos.ALL_Infos.sw_smpl_interval = ALL_SETTING_SW_SMPL_INTERVAL_DEFAULT;
    }
    // SW設定 デジタル一致検出回数
    if (my_all_infos.ALL_Infos.sw_check_count <= 0x0000)
    {
        my_all_infos.ALL_Infos.sw_check_count = ALL_SETTING_SW_CHECK_COUNT_DEFAULT;
    }
    // SW設定 ロータリーエンコーダ設定
    if (my_all_infos.ALL_Infos.sw_renc_conv_enables <= 0x0000)
    {
        my_all_infos.ALL_Infos.sw_renc_conv_enables = ALL_SETTING_SW_RENC_CONV_ENABLES_DEFAULT;
    }
    // SW設定 ロータリーエンコーダ用共通一致検出回数
    if (my_all_infos.ALL_Infos.sw_renc_check_count <= 0x0000)
    {
        my_all_infos.ALL_Infos.sw_renc_check_count = ALL_SETTING_SW_RENC_CHECK_COUNT_DEFAULT;
    }
    // AN設定 アナログサンプリング周期
    if (my_all_infos.ALL_Infos.an_smpl_interval <= 0x0000)
    {
        my_all_infos.ALL_Infos.an_smpl_interval = ALL_SETTING_AN_SMPL_INTERVAL_DEFAULT;
    }
}
