// Revive Advance
//
//  ver 1.0.2c  2021.04.15  カスタム版　アナログ入力およびデジタル入力でジョイスティックを使用した時、同時押し後の離し判定の入力が遅れることがある不具合を修正
//  ver 1.0.1c  2021.03.19  カスタム版　デジタル入力でジョイスティックに設定したピンの入力が不定期に途切れる不具合を修正
//  ver 1.0.0c  2020.09.30  カスタム版　リリース版
...
//    INTCONbits.MVEC = 0;    // enable single-vector mode
    ...
    for(fi = 0; fi < AD_INPUT_NUM; fi++)
    {
        ...
        ad_buffer[fi] = w_temp / AD_AVERAGE_NUM;
        ad_input_comp[fi] = 1;
    }
...
void ProcessIO(void)
{
    ...
    // アナログサンプリング周期に対し、64回実行(AD_INPUT_NUM=8、AD_AVERAGE_NUM=8のとき)
    if(ad_input_flag != 0)
    {
        ad_input_flag = 0;
        // AD入力
        ...
        ad_buffer[ad_input_no] = w_temp / AD_AVERAGE_NUM;
