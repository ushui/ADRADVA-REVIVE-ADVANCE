...
// USB Data
    ...
    if(!HIDRxHandleBusy(USBOutHandle))
    {
        ...
        switch(ReceivedDataBuffer[0])				//Look at the data the host sent, to see what kind of application specific command it sent.
        {
             ...
             * 全体 設定
             * [0x71                             -> [0x71
             * ,デジタルサンプリング周期         -> ,デジタルサンプリング周期
             * ,デジタル一致検出回数             -> ,デジタル一致検出回数
             * ,ロータリーエンコーダ設定         -> ,ロータリーエンコーダ設定
             * ,ロータリーエンコーダ一致検出回数 -> ,ロータリーエンコーダ一致検出回数
             * ,アナログサンプリング周期         -> ,アナログサンプリング周期
             * ]                                    ]
            */
            case 0x71:
                ToSendDataBuffer[0] = 0x71;
                
                if(ReceivedDataBuffer[1] == 0x00)
                {
                    if(ALL_SETTING_SW_SMPL_INTERVAL_MIN     <= ReceivedDataBuffer[2] && ReceivedDataBuffer[2] <= ALL_SETTING_SW_SMPL_INTERVAL_MAX
                    && ALL_SETTING_SW_CHECK_COUNT_MIN       <= ReceivedDataBuffer[3] && ReceivedDataBuffer[3] <= ALL_SETTING_SW_CHECK_COUNT_MAX
                    && ALL_SETTING_SW_RENC_CONV_ENABLES_MIN <= ReceivedDataBuffer[4] && ReceivedDataBuffer[4] <= ALL_SETTING_SW_RENC_CONV_ENABLES_MAX
                    && ALL_SETTING_SW_RENC_CHECK_COUNT_MIN  <= ReceivedDataBuffer[5] && ReceivedDataBuffer[5] <= ALL_SETTING_SW_RENC_CHECK_COUNT_MAX
                    && ALL_SETTING_AN_SMPL_INTERVAL_MIN     <= ReceivedDataBuffer[6] && ReceivedDataBuffer[6] <= ALL_SETTING_AN_SMPL_INTERVAL_MAX)
                    {
                        eeprom_address = EEPROM_ALL_SETTION_ADR + EEPROM_ALL_SETTION_SIZE;
                        b_temp_arry[0] = ReceivedDataBuffer[2];
                        b_temp_arry[1] = ReceivedDataBuffer[3];
                        b_temp_arry[2] = ReceivedDataBuffer[4];
                        b_temp_arry[3] = ReceivedDataBuffer[5];
                        b_temp_arry[4] = ReceivedDataBuffer[6];
                        my_all_infos.ALL_Infos.sw_smpl_interval     = b_temp_arry[0];
                        my_all_infos.ALL_Infos.sw_check_count       = b_temp_arry[1];
                        my_all_infos.ALL_Infos.sw_renc_conv_enables = b_temp_arry[2];
                        my_all_infos.ALL_Infos.sw_renc_check_count  = b_temp_arry[3];
                        my_all_infos.ALL_Infos.an_smpl_interval     = b_temp_arry[4];

                        eeprom_n_write(eeprom_address, &b_temp_arry[0], 5);
